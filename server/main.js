...
const USER_SQL = 
    {"SELECT_USER" : 
    "select userNo, userName, userId, userPwd, complete from user_info where  1=1 and #userId=userId#"};
...
var mysqlUtil = (sqlId, values)=>{
    var sql = USER_SQL[sqlId];
    try{
        if(!sql){
...
try{
    mysqlUtil("SELECT_USER",{'userId':'t'})
    .then((result)=>{
        console.log(result);
    });
}catch(e){
